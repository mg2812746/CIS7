 ...
 * Created on October 12th, 2021, 10:05 AM
...
float logAM(int a[],int n){
    ...
    logArtM/=static_cast<float>(n);
    //Log the resultant and return
    return log(logArtM);
}
...
float logGM(int a[],int n){
    //Declare variables
    float logGeoM=0;
    //Assign the log of the first entry to logGeoM
    logGeoM+=log(a[0]);
    //Compute the multiplicative total of the log for each entry
    for(int i=1;i<n;i++){
        logGeoM+=log(a[i]);
    }
    //Divide by the total
    logGeoM/=static_cast<float>(n);
    return logGeoM;
}
