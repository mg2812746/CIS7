...
int main(int argc, char** argv) {
    ...
    bool p,q;
    //Display the Heading Including Absorption Law
    cout<<"p q ~p ~q p&&q p||q p^q p^q^q p^q^p !(p&&q) !p||!q !(p||q) !p^!q"
        <<"   p||(p&&q)=p    p&&(p||q)=p"<<endl;
    //Display the Rows
    for(int i=0;i<=3;i++){
        ...
        cout<<(p?'T':'F')<<setw(2)                      //Column 1
            ...
            <<((!p^!q)?'T':'F')<<setw(8)                //Column 13
            <<((p||(p&&q))?'T':'F')<<setw(8)            //Column 14
            <<(p?'T':'F')<<setw(7)                      //Column 15
            <<(((p&&(p||q)))?'T':'F')<<setw(8)          //Column 16
            <<(p?'T':'F')                               //Column 17
            <<endl;
    }
    //Exit
