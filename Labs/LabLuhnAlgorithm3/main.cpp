 ...
 * Created on Sept 27, 2021, 5:19PM
 */
...
void trnsPse(char []);      //Randomly Swap or Transpose two numbers

int main(int argc, char** argv) {
    //Set the random number seed
    srand(static_cast<unsigned int>(time(0)));
    //Declare Variables Here
    CrdCard type=AMEX;          //Credit Card Type
    ...
    ptr=(genCC(type));  
    //Randomly Transpose two numbers
    trnsPse(ptr);
    //Randomly Flip Two Digits
    flipDig(ptr);
    //Tally valid cards
...
char *genCC(CrdCard type){
    ...
    switch(all){
        case AMEX:
            ptr= new char [16];
            ptr[0]='3';
            //second digit is either 7 or 4 50/50 chance
            rndDgit()<53?ptr[1]='7':    
                         ptr[1]='4';
            prpLuhn(ptr,14);
            Luhn(ptr,16);
...
//Randomly flip two digits from the CC number
...
void trnsPse(char cc[]){
    ...
    short rndElem1;
    short rndElem2;
    //Generate Two Random elements for swap
    rndElem1=rndDblDgit(cc);
    rndElem2=rndDblDgit(cc);
    //Swap both elements of array
    temp=cc[rndElem1];
    cc[rndElem1]=cc[rndElem2];
    cc[rndElem2]=temp;
}
